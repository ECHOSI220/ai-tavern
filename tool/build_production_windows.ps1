...
try {
 & flutter build windows --release '--dart-define=APP_ENV=production' "--dart-define=GAME_SERVER_URL=$($config['GAME_SERVER_URL'])" "--dart-define=SUPABASE_URL=$($config['SUPABASE_URL'])" "--dart-define=SUPABASE_ANON_KEY=$($config['SUPABASE_ANON_KEY'])"
